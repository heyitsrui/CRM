...
const Finance = () => {
  ...
  const handleSave = async (id) => {
    try {
      // Backend still performs (DB_Amount - paid_amount) = due_amount
      const res = await axios.put(`http://localhost:5000/api/finance/update/${id}`, {
      ...
      <table className="finance-table">
        ...
        <tbody>
          {projects
            ...
            .map((p) => {
              const totalContract = parseFloat(p.amount) || 0;
