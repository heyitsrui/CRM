...
// Helper to ensure the database has the correct columns before running
const initializeTimetreeDB = async () => {
    try {
        // Automatically add deadline_date if the user hasn't created it yet
        await queryDB(`
            ALTER TABLE timetree_events 
            ADD COLUMN IF NOT EXISTS deadline_date DATE DEFAULT NULL
        `);
        console.log("MariaDB: timetree_events schema verified.");
    } catch (err) {
        console.error("Schema Update Warning:", err.message);
    }
};
initializeTimetreeDB();
...
app.get("/api/timetree/events", async (req, res) => {
    try {
        ...
        const eventsWithChats = await Promise.all(events.map(async (event) => {
            const chats = await queryDB(
                "SELECT sender_name, message_text, sent_at FROM event_chats WHERE event_id = ? ORDER BY sent_at ASC",
                [event.id]
            );
            return { ...event, chats: chats || [] };
        }));
        res.json({ success: true, events: eventsWithChats });
    } catch (err) {
...
// 2. Create event - BULLETPROOF FORMATTING
app.post("/api/timetree/events", async (req, res) => {
    const { title, date, startTime, deadline_date } = req.body;

    try {
        // Fix 1: Ensure Time is HH:MM:00
        const finalTime = startTime && startTime.length === 5 ? `${startTime}:00` : startTime;

        // Fix 2: Convert empty strings or undefined to strict NULL
        const finalDeadline = (deadline_date && String(deadline_date).trim() !== "") ? deadline_date : null;

        // Fix 3: Ensure date is just YYYY-MM-DD
        const finalDate = date ? date.split('T')[0] : null;

        const result = await queryDB(
            "INSERT INTO timetree_events (title, event_date, start_time, deadline_date) VALUES (?, ?, ?, ?)",
            [title, finalDate, finalTime, finalDeadline]
        );
        
        res.json({ success: true, insertId: result.insertId });
    } catch (err) {
        // This log appears in your TERMINAL (Backend)
        console.error("MariaDB INSERT Error:", err.sqlMessage || err.message);
        
        // This sends the actual SQL error back to the frontend alert
        res.status(500).json({ 
            success: false, 
            error: err.sqlMessage || "Format Mismatch: Check if date/time are valid strings." 
        });
    }
});
...
app.delete("/api/timetree/events/:id", async (req, res) => {
    const { id } = req.params;
    try {
        // Delete chats first to maintain foreign key integrity
