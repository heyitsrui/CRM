  ...
  try {
    const rows = await queryDB(`
      SELECT id, deal_name, deal_owner, address, status, paid_amount, due_amount,
             description, contact, company, total_amount, created_at
      FROM projects
...
try {
    // Use total_amount from the request body
...
app.put("/api/finance/update/:id", async (req, res) => {
  ...
  try {
    ...
    const total_amount = parseFloat(projectRows[0].amount) || 0;
    const paidAmount = parseFloat(paid_amount) || 0;
    
    // 2. The Logic: Total - Paid = Due
    const remainingDue = total_amount - paidAmount;
  ...
  try {
    await queryDB(
      ...
      [
        ...
        contact || "", 
        company || "", 
        parseFloat(total_amount) || 0, // Update due_amount instead
        id
      ]
    );
...
app.put("/api/projects/:id", async (req, res) => {
  ...
  try {
    await queryDB(
      `UPDATE projects SET deal_name=?, deal_owner=?, address=?, description=?, contact=?, company=?, total_amount=? WHERE id=?`,
      [deal_name, deal_owner || "", address || "", description || "", contact || "", company || "", parseFloat(total_amount) || 0, id]
    );
    res.json({ success: true });
  } catch (err) {
    res.status(500).json({ success: false, error: err.message });
...
app.get("/api/finance/projects", async (req, res) => {
  try {
    const rows = await queryDB(`
      SELECT id, deal_name, company, deal_owner, paid_amount, due_amount, total_amount, status 
      FROM projects 
      ORDER BY created_at DESC
    `);
    res.json({ success: true, projects: rows });
...
app.put("/api/finance/update/:id", async (req, res) => {
  ...
  try {
    // 1. Corrected: Use 'total_amount' as seen in your HeidiSQL screenshot
    ...
    res.json({ success: true, message: "Finance updated successfully", balance: valDue });
  } catch (err) {
